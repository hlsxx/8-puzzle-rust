...
fn is_visited(stack: &Vec<Node>, current_node: Node) -> bool {
    let mut tmp_stack = stack.clone();
    let mut exist = false;

    while let Some(node) = tmp_stack.pop() {
...
impl Puzzle {
    ...
    pub fn DFS(&mut self) -> Result<(Node, usize), String> {
        ...
        while let Some(current_node) = self.stack.pop().as_mut() {
            ...
            for current_child in current_node.children.iter() {
                ...
                if !is_visited(&self.stack, current_child.clone()) && !is_visited(&self.visited, current_child.clone()) {
